            ...
            <b-form @submit="cadastrar" @reset="resetar" v-if="show" >
              <b-row>
                ...
                <b-col sm="12" lg="3">
                  ...
                  <b-tooltip placement="auto" target="grp-parcelas" v-if="$store.getters.getSatusTooltip">
                    Ao inserir um valor diferente de 1, o valor será dividido pelo número de parcelas e cada fração será inserida data do mês subsequente
                  </b-tooltip>
                </b-col>
              </b-row>
              <b-row>
...
export default {
  ...
  methods:{
    ...
    async cadastrar(event){
      ...
      if (this.produto === ''){
        ...
        if (this.form.parcelas > 1){
          ...
          const dataGasto = this.form.data
          for (let i = 0; i< this.form.parcelas; i++){
            this.form.valor = parcela.toString()
            //data Timestamp
            const dataTS = new Date(dataGasto)
            const dataTSparcela = new Date(dataTS.setMonth(dataTS.getMonth() + i))
            this.form.dataTS = firebase.firestore.Timestamp.fromDate(dataTSparcela)
            //data padrão
            this.form.data = dataTSparcela.toISOString().split('T')[0]
            await this.cadastraCusto()
          }
          this.resetar()
          this.getFields()
        } else if (this.form.parcelas === 1){
          ...
          this.mensagem = 'Número de parcelas inválido.'
          this.loading = false
          this.$refs['modal-err'].show()
        }
      }
    ...
    async cadastraCusto(){
      ...
      await this.$store.dispatch('setCustoOp',this.form)
          .then((retorno) => {
